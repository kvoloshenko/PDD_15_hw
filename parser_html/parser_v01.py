...
def get_news(topic):
    ...
    if topic == 'covid': url = f'{domain}/s/coronavirus/40/'
    else: url = f'{domain}/s/afisha/11/'
    print(f'url={url}')
    ...
    # print(f'parsed_data={parsed_data}')
    data_save_json(parsed_data,f'parsed_data_{topic}.json')
    return parsed_data

if __name__ == '__main__':
    result = get_news('covid')
    print(f'result={result}')
    result = get_news('where')
    print(f'result={result}')
